...
pub(crate) fn popup_origin(frame: &FrameData, popup: &FrameData, cell_width: f32) -> Point<Pixels> {
    point(
...
pub(crate) fn cursor_offset(cursor: &CursorState, cell_width: f32) -> Point<Pixels> {
    point(
...
pub(crate) fn input_cursor_bounds(
    ...
    if let Some(surface) = surface {
        let frame = if let Some(popup) = &surface.popup {
            origin += popup_origin(&surface.frame, &popup.frame, cell_width);
            &popup.frame
        } else {
            &surface.frame
        };
        if let Some(cursor) = &frame.cursor {
            origin += cursor_offset(cursor, cell_width);
        }
    }
...
impl WheelAccumulator {
    pub fn lines(&mut self, target: &InputTarget, event: &ScrollWheelEvent) -> i16 {
...
pub fn wheel_target(
    ...
    let (target, rect, pixel_mouse, width_px, height_px, origin_x, origin_y) =
        if let Some(popup) = &surface.popup {
            let origin = popup_origin(&surface.frame, &popup.frame, cell_width);
            (
...
mod tests {
    ...
    fn wheel_preserves_fractions_and_resets_on_target_direction_or_gesture_change() {
        ...
        assert_eq!(wheel.lines(&pane, &event), 0);
        assert_eq!(wheel.lines(&pane, &event), 1);
        assert_eq!(wheel.lines(&other, &event), 0);
        assert_eq!(wheel.lines(&popup, &event), 0);
        event.touch_phase = TouchPhase::Started;
        assert_eq!(wheel.lines(&popup, &event), 0);
        event.touch_phase = TouchPhase::Moved;
        event.delta = ScrollDelta::Lines(point(0., -1.));
        assert_eq!(wheel.lines(&popup, &event), -1);
        event.delta = ScrollDelta::Lines(point(0., 1e9));
        assert_eq!(wheel.lines(&popup, &event), 128);
    ...
    fn nonfinite_wheel_deltas_do_not_poison_fractional_motion() {
        ...
        for invalid in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
            ...
            assert_eq!(wheel.lines(&pane, &event), 1);
            event.delta = ScrollDelta::Lines(point(0., -1e9));
            assert_eq!(wheel.lines(&pane, &event), -128);
    ...
    fn wheel_hits_inner_pane_and_uses_relative_coordinates_and_semantic_modes() {
        ...
        for invalid in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY, 0., -1.] {
            assert!(wheel_target(&surface, 35., 65., invalid).is_none());
        }
        ...
        assert_eq!(input_cursor_bounds(None, origin, 8.5).origin, origin);
    }
    ...
    fn popup_origin_rounds_horizontal_pixels_and_saturates_oversized_frames() {
        ...
        assert_eq!(popup_origin(&frame, &popup, 8.5), point(px(259.), px(150.)));
        assert_eq!(popup_origin(&popup, &frame, 8.5), Point::default());
    }
