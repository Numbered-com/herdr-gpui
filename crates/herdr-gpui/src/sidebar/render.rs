...
use super::{
    ARROW_RESERVE, DEVICE_FOOTER_HEIGHT, HOST_ARROW_WIDTH, HOST_GAP, STATUS_WIDTH, SidebarDrag,
    agents::agent_labels,
    ...
    reorder::{self, Plan},
    row::first_text,
    row::{RowIcon, RowKind, RowLift, RowTree, removing_dot, row},
        ...
        for (endpoint_index, endpoint) in self.endpoints.iter().enumerate() {
            ...
            for (position, (index, indented, group)) in entries.into_iter().enumerate() {
                ...
                let element = row(
                    ...
                    workspace_badge(
                        workspace,
                        (endpoint_index == self.selected_endpoint).then_some(&self.menu.pr_cache),
                        &self.git,
                        theme,
                    ),
                    look,
