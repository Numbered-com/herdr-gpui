...
#[cfg(test)]
fn check_row_drag(style: crate::config::RowStyle, cx: &mut gpui::TestAppContext) {
    use gpui::{MouseButton, point};
...
fn superset_rows_lift_and_drop(cx: &mut gpui::TestAppContext) {
    check_row_drag(crate::config::RowStyle::Superset, cx);
}

#[gpui::test]
fn orca_rows_lift_and_drop(cx: &mut gpui::TestAppContext) {
    check_row_drag(crate::config::RowStyle::Orca, cx);
}
